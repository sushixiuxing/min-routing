...
#ifndef MIN_ROUTING_PROTOCOL_H
#define MIN_ROUTING_PROTOCOL_H
#include "ns3/test.h"
...
#include <vector>
#include <map>
#include "min-routing-header.h"
#include "min-routing-repositories.h"
#include "min-routing-state.h"

/// Testcase for MPR computation mechanism
class Min_routingMprTestCase;

namespace ns3 {
namespace min_routing {

///
/// \defgroup min_routing Min_Routing Routing
/// This section documents the API of the ns-3 Min_Routing module. For a generic
/// functional description, please refer to the ns-3 manual.

/// \ingroup min_routing
/// An %Min_Routing's routing table entry.
...
/// \ingroup min_routing
///
/// \brief Min_Routing routing protocol for IPv4
///
...
public:
   ...
   */
  friend class ::Min_routingMprTestCase;
  ...
  /**
   * \brief Set the Min_Routing main address to the first address on the indicated interface.
   *
   ...
   * \return the list of routing table entries discovered by Min_Routing
   */
...
public:
   ...
   * local HNA associations to be sent by the node via HNA messages.
   * If this tuple does not exist, nothing is done (see "Min_routingState::EraseAssociation()").
   *
   * \param networkAddr The network address.
   * \param netmask The network mask.
   */
   ...
   *         to the Min_Routing routing protocol. Entries from this associated
   *         routing table that use non-min_routing outgoing interfaces are added
...
private:
  ...
  double Range;               // the range of communication. I defined it!
  uint8_t m_willingness;  //!<  Willingness for forwarding packets on behalf of other nodes.

  Min_routingState m_state;  //!< Internal state with all needed data structs.
  ...
  double Predelay;             //I defined!!!  The next period delay
  /**
  ...
  /**
   * Receive an Min_Routing message.
   * \param socket The receiving socket.
   */
  void RecvMin_routing (Ptr<Socket> socket);
  ...
  /**
   *  \brief Tests whether or not the specified route uses a non-Min_Routing outgoing interface.
   *  \param route The route to be tested.
   *  \returns True if the outgoing interface of the specified route is a non-Min_Routing interface, false otherwise.
   */
  bool UsesNonMin_routingOutgoingInterface (const Ipv4RoutingTableEntry &route);
   ...
   *
   * The task of actually removing the tuple is left to the Min_Routing agent.
   *
  ...
  min_routing::MessageList m_queuedMessages;
  Timer m_queuedMessagesTimer; //!< timer for throttling outgoing messages

  /**
   * \brief Min_Routing's default forwarding algorithm.
   *
   * See \RFC{3626} for details.
   *
   * \param min_routingMessage The %Min_Routing message which must be forwarded.
   ...
   */
  void ForwardDefault (min_routing::MessageHeader min_routingMessage,
                       DuplicateTuple *duplicated,
                       const Ipv4Address &localIface,
                       const Ipv4Address &senderAddress);

  /**
   * \brief Enques an %Min_Routing message which will be sent with a delay of (0, delay].
   *
   * This buffering system is used in order to piggyback several %Min_Routing messages in
   * a same %Min_Routing packet.
   *
   * \param message the %Min_Routing message which must be sent.
   * \param delay maximum delay the %Min_Routing message is going to be buffered.
   */
  void QueueMessage (const min_routing::MessageHeader &message, Time delay);

  /**
   * \brief Creates as many %Min_Routing packets as needed in order to send all buffered
   * %Min_Routing messages.
   *
   * Maximum number of messages which can be contained in an %Min_Routing packet is
   * dictated by Min_Routing_MAX_MSGS constant.
   */
  void SendQueuedMessages ();

  /**
   * \brief Creates a new %Min_Routing HELLO message which is buffered for being sent later on.
   */
  void SendHello ();

  /**
   * \brief Creates a new %Min_Routing TC message which is buffered for being sent later on.
   */
  void SendTc ();

  /**
   * \brief Creates a new %Min_Routing MID message which is buffered for being sent later on.
   */
  void SendMid ();

  /**
   * \brief Creates a new %Min_Routing HNA message which is buffered for being sent later on.
   */
  void SendHna ();
   ...
   *
   * \param msg the %Min_Routing message which contains the HELLO message.
   ...
   */
  void ProcessHello (const min_routing::MessageHeader &msg,
   ...
   *
   * \param msg The %Min_Routing message which contains the TC message.
   * \param senderIface The address of the interface where the message was sent from.
   *
   */
  void ProcessTc (const min_routing::MessageHeader &msg,
                  const Ipv4Address &senderIface);
   ...
   *
   * \param msg the %Min_Routing message which contains the MID message.
   * \param senderIface the address of the interface where the message was sent from.
   */
  void ProcessMid (const min_routing::MessageHeader &msg,
                   const Ipv4Address &senderIface);
   ...
   *
   * \param msg the %Min_Routing message which contains the HNA message.
   * \param senderIface the address of the interface where the message was sent from.
   *
   */
  void ProcessHna (const min_routing::MessageHeader &msg,
                   const Ipv4Address &senderIface);
   ...
   */
  void LinkSensing (const min_routing::MessageHeader &msg,
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateNeighborSet (const min_routing::MessageHeader &msg,
                            const min_routing::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateTwoHopNeighborSet (const min_routing::MessageHeader &msg,
                                  const min_routing::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateMprSelectorSet (const min_routing::MessageHeader &msg,
                               const min_routing::MessageHeader::Hello &hello);
  ...
  // (reason: for Min_Routing Link Sensing we need to know on which interface
