...
#ifndef MIN_ROUTING_REPOSITORIES_H
#define MIN_ROUTING_REPOSITORIES_H
...
struct NeighborTuple
{
  ...
  uint16_t waitingdelay;  //I add it!
};

static inline bool
operator == (const NeighborTuple &a, const NeighborTuple &b)            //whether x_pos ect should add  into here need further condiderision???
{
...
operator << (std::ostream &os, const NeighborTuple &tuple)
{
  os << "NeighborTuple(neighborMainAddr=" << tuple.neighborMainAddr
	 ...
	 << ",position=("<<tuple.x_pos<<","<<tuple.y_pos<<")"
	 << ",velocity=("<<tuple.x_vel<<","<<tuple.y_vel<<"))";
  return os;
}
...
struct TwoHopNeighborTuple
{
  ...
  // I add!!! the waiting delay of the neighbor in this tuple
  uint16_t waitingdelay;
  /// Time at which this tuple expires and must be removed.
