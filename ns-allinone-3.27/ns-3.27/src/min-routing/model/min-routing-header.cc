...
#define Min_Routing_MSG_HEADER_SIZE 12
#define Min_Routing_PKT_HEADER_SIZE 4

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("Min_routingHeader");

namespace min_routing {


/// Scaling factor used in RFC 3626.
#define Min_Routing_C 0.0625
...
SecondsToEmf (double seconds)
{
  ...
  NS_ASSERT_MSG (seconds >= Min_Routing_C, "SecondsToEmf - Can not convert a value less than Min_Routing_C");

  // find the largest integer 'b' such that: T/C >= 2^b
  for (b = 1; (seconds / Min_Routing_C) >= (1 << b); ++b)
    {
    }
  NS_ASSERT ((seconds / Min_Routing_C) < (1 << b));
  b--;
  NS_ASSERT ((seconds / Min_Routing_C) >= (1 << b));

  // compute the expression 16*(T/(C*(2^b))-1), which may not be a integer
  double tmp = 16 * (seconds / (Min_Routing_C * (1 << b)) - 1);
...
///
/// \param min_routing_format number of seconds in mantissa/exponent format.
/// \return the decimal number of seconds.
///
double
EmfToSeconds (uint8_t min_routingFormat)
{
  int a = (min_routingFormat >> 4);
  int b = (min_routingFormat & 0xf);
  // value = C*(1+a/16)*2^b [in seconds]
  return Min_Routing_C * (1 + a / 16.0) * (1 << b);
}

// Conver int to uint, to write or read in buffer
uint8_t
IntToUint(int vel)
{
	uint8_t temp;
...
//convert uint to int, to write or read in buffer
int
UintToInt(uint8_t temp)
{
	int vel;
	NS_ASSERT(temp >=0 && temp <=256);
	if (temp <= 127)
	{
		vel=temp;
	}
...
{
  static TypeId tid = TypeId ("ns3::min_routing::PacketHeader")
    .SetParent<Header> ()
...
{
  return Min_Routing_PKT_HEADER_SIZE;
}
...
{
  static TypeId tid = TypeId ("ns3::min_routing::MessageHeader")
    .SetParent<Header> ()
    .SetGroupName ("Min_routing")
...
{
  uint32_t size = Min_Routing_MSG_HEADER_SIZE;
...
MessageHeader::Deserialize (Buffer::Iterator start)
{
  ...
  size = Min_Routing_MSG_HEADER_SIZE;
  switch (m_messageType)
    {
    case MID_MESSAGE:
      size += m_message.mid.Deserialize (i, m_messageSize - Min_Routing_MSG_HEADER_SIZE);
      break;
    case HELLO_MESSAGE:
      size += m_message.hello.Deserialize (i, m_messageSize - Min_Routing_MSG_HEADER_SIZE);
      break;
    case TC_MESSAGE:
      size += m_message.tc.Deserialize (i, m_messageSize - Min_Routing_MSG_HEADER_SIZE);
      break;
    case HNA_MESSAGE:
      size += m_message.hna.Deserialize (i, m_messageSize - Min_Routing_MSG_HEADER_SIZE);
      break;
    default:
      NS_ASSERT (false);
    }
...
MessageHeader::Hello::Serialize (Buffer::Iterator start) const
{
  ...
  i.WriteU16 (this->x_pos); //  add
  i.WriteU8 (this->hTime);
  i.WriteU8 (this->willingness);
  i.WriteU16 (this->y_pos); //add ,y_pos
  i.WriteU8 (this->x_vel);  //add
  i.WriteU8 (this->y_vel);  //add
...
MessageHeader::Hello::Deserialize (Buffer::Iterator start, uint32_t messageSize)
{
  ...
  this->x_pos= i.ReadU16();
  this->hTime = i.ReadU8 ();
  this->willingness = i.ReadU8 ();
  helloSizeLeft -= 4;
  this->y_pos= i.ReadU16();
  this->x_vel= i.ReadU8();
  this->y_vel= i.ReadU8();
  helloSizeLeft -= 4;
