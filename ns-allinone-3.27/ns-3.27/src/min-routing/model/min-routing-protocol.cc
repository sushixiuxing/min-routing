...
///
/// \brief Implementation of Min_Routing agent and related classes.
///
/// This is the main file of this software because %Min_Routing's behaviour is
/// implemented here.
///
...
#include "min-routing-protocol.h"
...
///
/// We only use this value in order to define Min_Routing_NEIGHB_HOLD_TIME.
///
#define Min_Routing_REFRESH_INTERVAL   m_helloInterval


/********** Holding times **********/

/// Neighbor holding time.
#define Min_Routing_NEIGHB_HOLD_TIME   Time (3 * Min_Routing_REFRESH_INTERVAL)
/// Top holding time.
#define Min_Routing_TOP_HOLD_TIME      Time (3 * m_tcInterval)
/// Dup holding time.
#define Min_Routing_DUP_HOLD_TIME      Seconds (30)
/// MID holding time.
#define Min_Routing_MID_HOLD_TIME      Time (3 * m_midInterval)
/// HNA holding time.
#define Min_Routing_HNA_HOLD_TIME      Time (3 * m_hnaInterval)

/********** Link types **********/

/// Unspecified link type.
#define Min_Routing_UNSPEC_LINK        0
/// Asymmetric link type.
#define Min_Routing_ASYM_LINK          1
/// Symmetric link type.
#define Min_Routing_SYM_LINK           2
/// Lost link type.
#define Min_Routing_LOST_LINK          3

/********** Neighbor types **********/

/// Not neighbor type.
#define Min_Routing_NOT_NEIGH          0
/// Symmetric neighbor type.
#define Min_Routing_SYM_NEIGH          1
/// Asymmetric neighbor type.
#define Min_Routing_MPR_NEIGH          2


/********** Willingness **********/

/// Willingness for forwarding packets from other nodes: never.
#define Min_Routing_WILL_NEVER         0
/// Willingness for forwarding packets from other nodes: low.
#define Min_Routing_WILL_LOW           1
/// Willingness for forwarding packets from other nodes: medium.
#define Min_Routing_WILL_DEFAULT       3
/// Willingness for forwarding packets from other nodes: high.
#define Min_Routing_WILL_HIGH          6
/// Willingness for forwarding packets from other nodes: always.
#define Min_Routing_WILL_ALWAYS        127


/********** Miscellaneous constants **********/

/// Maximum allowed jitter.
#define Min_Routing_MAXJITTER          (m_helloInterval.GetSeconds () / 4)
/// Maximum allowed sequence number.
#define Min_Routing_MAX_SEQ_NUM        65535
/// Random number between [0-Min_Routing_MAXJITTER] used to jitter Min_Routing packet transmission.
#define JITTER (Seconds (m_uniformRandomVariable->GetValue (0, Min_Routing_MAXJITTER)))


#define Min_Routing_PORT_NUMBER 698
/// Maximum number of messages per packet.
#define Min_Routing_MAX_MSGS           64

/// Maximum number of hellos per message (4 possible link types * 3 possible nb types).
#define Min_Routing_MAX_HELLOS         12

/// Maximum number of addresses advertised on a message.
#define Min_Routing_MAX_ADDRS          64


namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("Min_routingRoutingProtocol");

namespace min_routing {
...
{
  static TypeId tid = TypeId ("ns3::min_routing::RoutingProtocol")
    .SetParent<Ipv4RoutingProtocol> ()
    .SetGroupName ("Min_routing")
    .AddConstructor<RoutingProtocol> ()
    .AddAttribute ("HelloInterval", "HELLO messages emission interval.",
                   TimeValue (Seconds (2)),//orinial 2,I modified
    ...
    .AddAttribute ("HnaInterval", "HNA messages emission interval.  Normally it is equal to TcInterval.",
                   ...
                   MakeTimeChecker ())
	.AddAttribute ("R","The range of the communication.",                //I add!
			       DoubleValue (200),
				   MakeDoubleAccessor (&RoutingProtocol::Range),
				   MakeDoubleChecker<double> ())
    .AddAttribute ("Willingness", "Willingness of a node to carry and forward traffic for other nodes.",
                   EnumValue (Min_Routing_WILL_DEFAULT),
                   MakeEnumAccessor (&RoutingProtocol::m_willingness),
                   MakeEnumChecker (Min_Routing_WILL_NEVER, "never",
                                    Min_Routing_WILL_LOW, "low",
                                    Min_Routing_WILL_DEFAULT, "default",
                                    Min_Routing_WILL_HIGH, "high",
                                    Min_Routing_WILL_ALWAYS, "always"))
    .AddTraceSource ("Rx", "Receive Min_Routing packet.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_rxPacketTrace),
                     "ns3::min_routing::RoutingProtocol::PacketTxRxTracedCallback")
    .AddTraceSource ("Tx", "Send Min_Routing packet.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_txPacketTrace),
                     "ns3::min_routing::RoutingProtocol::PacketTxRxTracedCallback")
    .AddTraceSource ("RoutingTableChanged", "The Min_Routing routing table has changed.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_routingTableChanged),
                     "ns3::min_routing::RoutingProtocol::TableChangeTracedCallback")
  ;
  return tid;
}
...
RoutingProtocol::~RoutingProtocol ()
{
	//NS_LOG_UNCOND(RoutingProtocol::txbytes); //I defined it!!!
	//overhead+=txbytes;
	//NS_LOG_UNCOND(RoutingProtocol::overhead);         //I defined it!!!
	//std::string m_CSVfileName = "overhead_H2_T5_L1200W400_.csv";
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  NS_ASSERT (ipv4 != 0);
  NS_ASSERT (m_ipv4 == 0);
  NS_LOG_DEBUG ("Created min_routing::RoutingProtocol");
  ...
  m_packetSequenceNumber = Min_Routing_MAX_SEQ_NUM;
  m_messageSequenceNumber = Min_Routing_MAX_SEQ_NUM;
  m_ansn = Min_Routing_MAX_SEQ_NUM;
...
RoutingProtocol::PrintRoutingTable (Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
  ...
  *os << "Node: " << m_ipv4->GetObject<Node> ()->GetId ()
      ...
      << ", Min_Routing Routing table" << std::endl;
...
void RoutingProtocol::DoInitialize ()
{
  ...
  NS_LOG_DEBUG ("Starting Min_Routing on node " << m_mainAddress);

  Ipv4Address loopback ("127.0.0.1");

  bool canRunMin_routing = false;
  for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
    {
      Ipv4Address addr = m_ipv4->GetAddress (i, 0).GetLocal ();
      if (addr == loopback)
      ...
      if (m_recvSocket == 0)
        {
          ...
          InetSocketAddress inetAddr (Ipv4Address::GetAny (), Min_Routing_PORT_NUMBER);
          m_recvSocket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvMin_routing,  this));
          if (m_recvSocket->Bind (inetAddr))
            {
              NS_FATAL_ERROR ("Failed to bind() Min_Routing socket");
            }
      ...
      InetSocketAddress inetAddr (m_ipv4->GetAddress (i, 0).GetLocal (), Min_Routing_PORT_NUMBER);
      socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvMin_routing,  this));
      ...
      canRunMin_routing = true;
    }

  if (canRunMin_routing)
    {
      HelloTimerExpire ();
      TcTimerExpire ();
      MidTimerExpire ();
      ...
      NS_LOG_DEBUG ("Min_Routing on node " << m_mainAddress << " started");
    }
}
...
//
// \brief Processes an incoming %Min_Routing packet following \RFC{3626} specification.
void
RoutingProtocol::RecvMin_routing (Ptr<Socket> socket)
{
  ...
  if (!receivedPacket->RemovePacketTag (interfaceInfo))
    {
      NS_ABORT_MSG ("No incoming interface on Min_Routing message, aborting.");
    }
  ...
  NS_LOG_DEBUG ("Min_Routing node " << m_mainAddress << " received a Min_Routing packet from "
                             << senderIfaceAddr << " to " << receiverIfaceAddr);

  // All routing messages are sent from and to port RT_PORT,
  // so we check it.
  NS_ASSERT (inetSourceAddr.GetPort () == Min_Routing_PORT_NUMBER);

  Ptr<Packet> packet = receivedPacket;

  min_routing::PacketHeader min_routingPacketHeader;
  packet->RemoveHeader (min_routingPacketHeader);
  NS_ASSERT (min_routingPacketHeader.GetPacketLength () >= min_routingPacketHeader.GetSerializedSize ());
  uint32_t sizeLeft = min_routingPacketHeader.GetPacketLength () - min_routingPacketHeader.GetSerializedSize ();
  ...
  while (sizeLeft)
    {
      ...
      NS_LOG_DEBUG ("Min_routing Msg received with type "
  ...
  m_rxPacketTrace (min_routingPacketHeader, messages);
...
//         }
      ...
      if (duplicated == NULL)
        {
          switch (messageHeader.GetMessageType ())
            {
            case min_routing::MessageHeader::HELLO_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s Min_Routing node " << m_mainAddress
            ...
            case min_routing::MessageHeader::TC_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s Min_Routing node " << m_mainAddress
            ...
            default:
              NS_LOG_DEBUG ("Min_Routing message type " <<
        ...
        {
          NS_LOG_DEBUG ("Min_Routing message is duplicated, not reading it.");
      ...
      if (do_forwarding)
        {
          ...
          if (messageHeader.GetMessageType ()  != min_routing::MessageHeader::HELLO_MESSAGE)
            {
  ...
  // After processing all Min_Routing messages, we must recompute the routing table
...
RoutingProtocol::MprComputation ()
{
      ...
      for (NeighborSet::const_iterator neigh = N.begin ();
           neigh != N.end (); neigh++)
        {
          if (neigh->neighborMainAddr == twoHopNeigh->neighborMainAddr)
            {
              if (neigh->willingness == Min_Routing_WILL_NEVER)
                {
                  ok = false;
                  break;
                }
  ...
  /*for (NeighborSet::const_iterator neighbor = N.begin (); neighbor != N.end (); neighbor++)
    {
      if (neighbor->willingness == Min_Routing_WILL_ALWAYS)
        {
...
void
RoutingProtocol::ProcessHello (const min_routing::MessageHeader &msg,
                               const Ipv4Address &receiverIface,
                               const Ipv4Address &senderIface)
{
  NS_LOG_FUNCTION (msg << receiverIface << senderIface);

  const min_routing::MessageHeader::Hello &hello = msg.GetHello ();
...
#ifdef NS3_LOG_ENABLE
  {
    ...
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump Link Set for Min_Routing Node " << m_mainAddress);
    for (LinkSet::const_iterator link = links.begin (); link != links.end (); link++)
      {
        NS_LOG_DEBUG (*link);
      }
    NS_LOG_DEBUG ("** END dump Link Set for Min_Routing Node " << m_mainAddress);

    const NeighborSet &neighbors = m_state.GetNeighbors ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump Neighbor Set for Min_Routing Node " << m_mainAddress);
    for (NeighborSet::const_iterator neighbor = neighbors.begin (); neighbor != neighbors.end (); neighbor++)
      {
        NS_LOG_DEBUG (*neighbor);
      }
    NS_LOG_DEBUG ("** END dump Neighbor Set for Min_Routing Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
...
#ifdef NS3_LOG_ENABLE
  {
    const TwoHopNeighborSet &twoHopNeighbors = m_state.GetTwoHopNeighbors ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump TwoHopNeighbor Set for Min_Routing Node " << m_mainAddress);
    for (TwoHopNeighborSet::const_iterator tuple = twoHopNeighbors.begin ();
         tuple != twoHopNeighbors.end (); tuple++)
      {
        NS_LOG_DEBUG (*tuple);
      }
    NS_LOG_DEBUG ("** END dump TwoHopNeighbor Set for Min_Routing Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
...
void
RoutingProtocol::ProcessTc (const min_routing::MessageHeader &msg,
                            const Ipv4Address &senderIface)
{
  const min_routing::MessageHeader::Tc &tc = msg.GetTc ();
...
#ifdef NS3_LOG_ENABLE
  {
    const TopologySet &topology = m_state.GetTopologySet ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump TopologySet for Min_Routing Node " << m_mainAddress);
    for (TopologySet::const_iterator tuple = topology.begin ();
         tuple != topology.end (); tuple++)
      {
        NS_LOG_DEBUG (*tuple);
      }
    NS_LOG_DEBUG ("** END dump TopologySet Set for Min_Routing Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
}

void
RoutingProtocol::ProcessMid (const min_routing::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{
  const min_routing::MessageHeader::Mid &mid = msg.GetMid ();
  Time now = Simulator::Now ();
...
void
RoutingProtocol::ProcessHna (const min_routing::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{

  const min_routing::MessageHeader::Hna &hna = msg.GetHna ();
  Time now = Simulator::Now ();
  ...
  for (std::vector<min_routing::MessageHeader::Hna::Association>::const_iterator it = hna.associations.begin ();
...
void
RoutingProtocol::ForwardDefault (min_routing::MessageHeader min_routingMessage,
                                 ...
                                 const Ipv4Address &senderAddress)
{
  ...
  if (duplicated != NULL && duplicated->retransmitted)
    {
      NS_LOG_LOGIC (Simulator::Now () << "Node " << m_mainAddress << " does not forward a message received"
                    " from " << min_routingMessage.GetOriginatorAddress () << " because it is duplicated");
      return;
    }
  ...
  if (min_routingMessage.GetTimeToLive () > 1)
    {
      const MprSelectorTuple *mprselTuple =
        m_state.FindMprSelectorTuple (GetMainAddress (senderAddress));
      if (mprselTuple != NULL)
        {
          min_routingMessage.SetTimeToLive (min_routingMessage.GetTimeToLive () - 1);
          min_routingMessage.SetHopCount (min_routingMessage.GetHopCount () + 1);
          // We have to introduce a random delay to avoid
          // synchronization with neighbors.
          QueueMessage (min_routingMessage, JITTER);
          retransmitted = true;
        }
    }
  ...
  if (duplicated != NULL)
    {
      duplicated->expirationTime = now + Min_Routing_DUP_HOLD_TIME;
  ...
  // ...or create a new one
  else
    {
      ...
      newDup.address = min_routingMessage.GetOriginatorAddress ();
      newDup.sequenceNumber = min_routingMessage.GetMessageSequenceNumber ();
      newDup.expirationTime = now + Min_Routing_DUP_HOLD_TIME;
      ...
      Simulator::Schedule (Min_Routing_DUP_HOLD_TIME,
...
void
RoutingProtocol::QueueMessage (const min_routing::MessageHeader &message, Time delay)
{
...
{
  NS_LOG_DEBUG ("Min_Routing node " << m_mainAddress << " sending a Min_Routing packet");

  // Add a header
  min_routing::PacketHeader header;
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
         m_sendSockets.begin (); i != m_sendSockets.end (); i++)
    {
      ...
      i->first->SendTo (pkt, 0, InetSocketAddress (bcast, Min_Routing_PORT_NUMBER));
      txbytes+=pkt->GetSize();   //I defined it!!!
    }
 // NS_LOG_UNCOND(txbytes); //I defined it!!!
}
...
RoutingProtocol::SendQueuedMessages ()              //To add several messages to a packet , I understand
{
  ...
  NS_LOG_DEBUG ("Min_routing node " << m_mainAddress << ": SendQueuedMessages");

  MessageList msglist;

  for (std::vector<min_routing::MessageHeader>::const_iterator message = m_queuedMessages.begin ();
       ...
       message++)
    {
      ...
      if (++numMessages == Min_Routing_MAX_MSGS)
        {
...
RoutingProtocol::SendHello ()
{
  ...
  min_routing::MessageHeader msg;
  Time now = Simulator::Now ();

  msg.SetVTime (Min_Routing_NEIGHB_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (1);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  min_routing::MessageHeader::Hello &hello = msg.GetHello ();

  hello.SetHTime (m_helloInterval);
  hello.willingness = m_willingness;

  //add, Set mobility and velocity
  Ptr<MobilityModel> model= m_ipv4->GetObject<Node>()->GetObject<MobilityModel>();
  Vector pos=model->GetPosition();
  hello.x_pos=uint16_t(pos.x);
  hello.y_pos=uint16_t(pos.y);
  Vector vel=model->GetVelocity();
  int lablex=round(vel.x);        //convert to the closest integer
  int labley=round(vel.y);
  hello.x_vel=IntToUint(lablex);
  hello.y_vel=IntToUint(labley);

 /* NS_LOG_UNCOND("MainAddress:"<<m_mainAddress<<"Position=("
		  <<hello.x_pos<<","<<hello.y_pos<<")"<<"Velocity=("<<UintToInt(hello.x_vel)<<","
		  <<UintToInt(hello.y_vel)<<")"); */

  std::vector<min_routing::MessageHeader::Hello::LinkMessage>
                  ...
                  if (nb_tuple->status == NeighborTuple::STATUS_SYM)
                    {
                      NS_LOG_DEBUG ("I consider neighbor " << GetMainAddress (link_tuple->neighborIfaceAddr)
                                                           << " to be SYM_NEIGH.");
                      nb_type = Min_Routing_SYM_NEIGH;
                    }
                  else if (nb_tuple->status == NeighborTuple::STATUS_NOT_SYM)
                    {
                      nb_type = Min_Routing_NOT_NEIGH;
                      NS_LOG_DEBUG ("I consider neighbor " << GetMainAddress (link_tuple->neighborIfaceAddr)
                                                           << " to be NOT_NEIGH.");
                    }
                  else
  ...
  NS_LOG_DEBUG ("Min_Routing HELLO message size: " << int (msg.GetSerializedSize ())
...
RoutingProtocol::SendTc ()
{
  ...
  msg.SetVTime (Min_Routing_TOP_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
...
RoutingProtocol::SendMid ()
{
  ...
  msg.SetVTime (Min_Routing_MID_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
...
RoutingProtocol::SendHna ()
{
  ...
  msg.SetVTime (Min_Routing_HNA_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  min_routing::MessageHeader::Hna &hna = msg.GetHna ();

  std::vector<min_routing::MessageHeader::Hna::Association> &associations = hna.associations;
  ...
  for (Associations::const_iterator it = localHnaAssociations.begin ();
    ...
    {
      min_routing::MessageHeader::Hna::Association assoc = { it->networkAddr, it->netmask};
...
RoutingProtocol::SetRoutingTableAssociation (Ptr<Ipv4StaticRouting> routingTable)
{
      ...
      for (uint32_t i = 0; i < m_routingTableAssociation->GetNRoutes (); i++)
        {
          ...
          // If the outgoing interface for this route is a non-min_routing interface
          if (UsesNonMin_routingOutgoingInterface (route))
            {
  ...
  // add the routes using non-min_routing outgoing interfaces to the list
...
bool
RoutingProtocol::UsesNonMin_routingOutgoingInterface (const Ipv4RoutingTableEntry &route)
{
  std::set<uint32_t>::const_iterator ci = m_interfaceExclusions.find (route.GetInterface ());
  // The outgoing interface is a non-Min_Routing interface if a match is found
  // before reaching the end of the list of excluded interfaces
  return ci != m_interfaceExclusions.end ();
}

void
RoutingProtocol::LinkSensing (const min_routing::MessageHeader &msg,
                              const min_routing::MessageHeader::Hello &hello,
                              const Ipv4Address &receiverIface,
                              const Ipv4Address &senderIface)
{
  Time now = Simulator::Now ();
  bool updated = false;
  bool created = false;
  NS_LOG_DEBUG ("@" << now.GetSeconds () << ": Min_routing node " << m_mainAddress
  ...
  for (std::vector<min_routing::MessageHeader::Hello::LinkMessage>::const_iterator linkMessage =
       ...
       linkMessage++)
    {
      ...
      switch (lt)
        {
        case Min_Routing_UNSPEC_LINK:
          linkTypeName = "UNSPEC_LINK";
          break;
        case Min_Routing_ASYM_LINK:
          linkTypeName = "ASYM_LINK";
          break;
        case Min_Routing_SYM_LINK:
          linkTypeName = "SYM_LINK";
          break;
        case Min_Routing_LOST_LINK:
      ...
      switch (nt)
        {
        case Min_Routing_NOT_NEIGH:
          neighborTypeName = "NOT_NEIGH";
          break;
        case Min_Routing_SYM_NEIGH:
          neighborTypeName = "SYM_NEIGH";
          break;
        case Min_Routing_MPR_NEIGH:
      ...
      if ((lt == Min_Routing_SYM_LINK && nt == Min_Routing_NOT_NEIGH)
          || (nt != Min_Routing_SYM_NEIGH && nt != Min_Routing_MPR_NEIGH
              && nt != Min_Routing_NOT_NEIGH))
        {
      ...
      for (std::vector<Ipv4Address>::const_iterator neighIfaceAddr =
           ...
           neighIfaceAddr++)
        {
          ...
          if (*neighIfaceAddr == receiverIface)
            {
              if (lt == Min_Routing_LOST_LINK)
                {
                  NS_LOG_LOGIC ("link is LOST => expiring it");
                  link_tuple->symTime = now - Seconds (1);
                  updated = true;
                }
              else if (lt == Min_Routing_SYM_LINK || lt == Min_Routing_ASYM_LINK)
                {
                  ...
                  link_tuple->time = link_tuple->symTime + Min_Routing_NEIGHB_HOLD_TIME;
                  updated = true;
                }
              else
                {
...
void
RoutingProtocol::PopulateNeighborSet (const min_routing::MessageHeader &msg,
                                      const min_routing::MessageHeader::Hello &hello)
{
  NeighborTuple *nb_tuple = m_state.FindNeighborTuple (msg.GetOriginatorAddress ());
  if (nb_tuple != NULL)
    {
	   //get this node's position and velocity
	   Ptr<MobilityModel> model= m_ipv4->GetObject<Node>()->GetObject<MobilityModel>();
	   Vector pos=model->GetPosition();
	   uint16_t x_mpos=uint16_t(pos.x);
	   uint16_t y_mpos=uint16_t(pos.y);
	   Vector vel=model->GetVelocity();
	   int x_mvel=round(vel.x);        //convert to the closest integer
	   int y_mvel=round(vel.y);

	   //get the neighbor's position and velocity
	   uint16_t x_npos=hello.x_pos;
	   uint16_t y_npos=hello.y_pos;
	   int x_nvel=UintToInt(hello.x_vel);
	   int y_nvel=UintToInt(hello.y_vel);

	   //Calculate the LLT
	   int mi=2;                           //mi yun suan
	   int N_willingness=3;
       double d=sqrt(pow((x_mpos-x_npos),mi)+pow((y_mpos-y_mpos),mi));       //distance
       double rvel=sqrt(pow((x_mvel-x_nvel),mi)+pow((y_mpos-y_npos),mi));    //relative velocity
       int x_rvel=x_nvel-x_mvel;                                //x lable of relative velovity
       ...
       uint8_t u_willingness=IntToUint(N_willingness);

       //NS_LOG_UNCOND("NeighborIP:"<<msg.GetOriginatorAddress()<<"u_willingness:"<<u_willingness<<"distangce:"<<d<<"Time:"
...
void
RoutingProtocol::PopulateTwoHopNeighborSet (const min_routing::MessageHeader &msg,
                                            const min_routing::MessageHeader::Hello &hello)
{
  Time now = Simulator::Now ();

  NS_LOG_DEBUG ("Min_routing node " << m_mainAddress << ": PopulateTwoHopNeighborSet BEGIN");
  ...
  for (LinkSet::const_iterator link_tuple = m_state.GetLinks ().begin ();
       link_tuple != m_state.GetLinks ().end (); link_tuple++)
    {
      ...
      typedef std::vector<min_routing::MessageHeader::Hello::LinkMessage> LinkMessageVec;
           ...
           linkMessage != hello.linkMessages.end (); linkMessage++)
        {
          ...
          for (std::vector<Ipv4Address>::const_iterator nb2hop_addr_iter =
               ...
               nb2hop_addr_iter++)
            {
              ...
              if (neighborType == Min_Routing_SYM_NEIGH || neighborType == Min_Routing_MPR_NEIGH)
                {
              ...
              else if (neighborType == Min_Routing_NOT_NEIGH)
                {
...
void
RoutingProtocol::PopulateMprSelectorSet (const min_routing::MessageHeader &msg,
                                         const min_routing::MessageHeader::Hello &hello)
{
  NS_LOG_FUNCTION (this);
...
///
void
min_routing::mac_failed (Ptr<Packet> p)
{
  ...
  debug ("%f: Node %d MAC Layer detects a breakage on link to %d\n",
         now,
         min_routing::node_id (ra_addr ()),
         min_routing::node_id (ch->next_hop ()));
  ...
  Min_Routing_link_tuple* link_tuple = state_.find_link_tuple (ch->next_hop ());
  if (link_tuple != NULL)
    {
      link_tuple->lost_time () = now + Min_Routing_NEIGHB_HOLD_TIME;
      link_tuple->time ()      = now + Min_Routing_NEIGHB_HOLD_TIME;
      nb_loss (link_tuple);
    }
  drop (p, DROP_RTR_MAC_CALLBACK);
}
...
RoutingProtocol::NeighborLoss (const LinkTuple &tuple)
{
  ...
  LinkTupleUpdated (tuple, Min_Routing_WILL_DEFAULT);
...
RoutingProtocol::AddDuplicateTuple (const DuplicateTuple &tuple)
{
  /*debug("%f: Node %d adds dup tuple: addr = %d seq_num = %d\n",
          Simulator::Now (),
          min_routing::node_id(ra_addr()),
          min_routing::node_id(tuple->addr()),
          tuple->seq_num());*/
...
//         Simulator::Now (),
//         min_routing::node_id(ra_addr()),
//         min_routing::node_id(tuple->neighborMainAddr),
//         ((tuple->status() == Min_Routing_STATUS_SYM) ? "sym" : "not_sym"));
...
//         min_routing::node_id(ra_addr()),
//         min_routing::node_id(tuple->neighborMainAddr),
//         min_routing::node_id(tuple->twoHopNeighborAddr));
...
{
  m_ansn = (m_ansn + 1) % (Min_Routing_MAX_SEQ_NUM + 1);
}
...
//         Simulator::Now (),
//         min_routing::node_id(ra_addr()),
//         min_routing::node_id(tuple->main_addr()));
...
//         Simulator::Now (),
//         min_routing::node_id(ra_addr()),
//         min_routing::node_id(tuple->dest_addr()),
//         min_routing::node_id(tuple->last_addr()),
//         tuple->seq());
...
//         min_routing::node_id(ra_addr()),
//         min_routing::node_id(tuple->main_addr()),
//         min_routing::node_id(tuple->iface_addr()));
...
{
  m_packetSequenceNumber = (m_packetSequenceNumber + 1) % (Min_Routing_MAX_SEQ_NUM + 1);
  return m_packetSequenceNumber;
}

uint16_t RoutingProtocol::GetMessageSequenceNumber ()
{
  m_messageSequenceNumber = (m_messageSequenceNumber + 1) % (Min_Routing_MAX_SEQ_NUM + 1);
  ...
  if (Lookup (header.GetDestination (), entry1) != 0)
    {
      ...
      if (oif && m_ipv4->GetInterfaceForDevice (oif) != static_cast<int> (interfaceIdx))
        {
          ...
          NS_LOG_DEBUG ("Min_routing node " << m_mainAddress
      ...
      if (numOifAddresses == 1)
        ...
        {
          /// \todo Implment IP aliasing and Min_Routing
          NS_FATAL_ERROR ("XXX Not implemented yet:  IP aliasing and Min_Routing");
        }
