...
WifiMacQueue::WifiMacQueue ():
    m_totalwaitingtime(NanoSeconds(0)),
	m_totalpackets(0),
    NS_LOG_TEMPLATE_DEFINE ("WifiMacQueue")
{
//	NS_LOG_UNCOND("SimulationTime:"<<Simulator::Now()<<"CeShi"); //I add!
}

WifiMacQueue::~WifiMacQueue ()
{
//	NS_LOG_UNCOND("The waitingdelay is:"<<GettotalWaitingtime());    //I add!!!
//	NS_LOG_UNCOND("SimulationTime:"<<Simulator::Now()<<"The total packets: "<<Gettotalpackets());
  m_totalwaitingtime=NanoSeconds(0);
  m_totalpackets=0;
...
//
//    Time WifiMacQueue::m_totalwaitingtime = NanoSeconds(0) ;          //I add! the waiting time of a packet
//    uint32_t WifiMacQueue::m_totalpackets = 0;
Time
WifiMacQueue::GettotalWaitingtime(void)     //I add! To get the total waiting time of all packets for IP routing.
{
	return m_totalwaitingtime;
}

uint32_t
WifiMacQueue::Gettotalpackets()
{
  ...
  for (auto it = Head (); it != Tail (); )
    {
	  ...
	  {
         m_totalwaitingtime+=Simulator::Now()-(*it)->GetTimeStamp();
         m_totalpackets++;
//         NS_LOG_UNCOND("SimulationTime:"<<Simulator::Now()<<"  "<<"The waitingdelay is:"
//     		 <<GettotalWaitingtime());    //I add!!!
	  }
      if (!TtlExceeded (it))
        {
          return DoDequeue (it);
      ...
      if (!TtlExceeded (it))
        {
          if (!(*it)->GetHeader ().IsQosData ()
            ...
            {
//        	  NS_LOG_UNCOND("The waitingdelay is:"<<Simulator::Now()-(*it)->GetTimeStamp ());    //I add!!!
              return DoDequeue (it);
            }
